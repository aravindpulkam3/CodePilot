...
import type { RetrievedContext } from "../../infrastructure/retrieval/retrievalTypes.js";
...
/** The subset of GitHub PR data prompt assembly actually reads. */
export interface ReviewPrInput {
  title: string;
  description: string | null;
  /** PR head commit. The patches' + line numbers refer to this revision. */
...
/** The subset of the repository row prompt assembly actually reads. */
export type ReviewRepoInput = Pick<
  RepositoryRow,
  "owner" | "name" | "default_branch" | "last_indexed_sha"
>;

/** Real I/O for changedCodeContext: GitHub content at a ref + the index's own AST chunker. */
export function githubChangedCodeDeps(
  owner: string,
  repo: string,
  token?: string,
): ChangedCodeDeps {
  return {
    fetchContent: (path, ref) =>
      githubService.fetchRawFileContent(token, owner, repo, path, ref),
    chunkFile: (path, content) => astChunker.chunkFile(path, content),
    supportsFile: (path) => astChunker.supportsFile(path),
  };
}
...
export class ReviewService {
  ...
  /**
   * Retrieval + prompt assembly for one review, with no LLM call and no
   * database writes. Split out of generateAndStoreReview so the exact prompt
   * a review would send can be inspected on its own (scripts/evalRetrieval.ts).
   */
  async buildReviewMessages(
    clerkUserId: string,
    repositoryId: string,
    repoDetails: ReviewRepoInput,
    prDetails: ReviewPrInput,
    extra?: { skipSync?: boolean; githubToken?: string },
  ): Promise<{
    messages: LLMMessage[];
    retrievedContext: RetrievedContext;
    changedCode: ChangedCodeContext | null;
  }> {
    // Post-change enclosing code for each changed file, from the PR HEAD —
    // the index holds the default branch, so it cannot be matched against the
    // patch's + line numbers. See changedCodeContext.ts. Files the prompt
    // never reviews (lockfiles, minified, binary) are left out entirely.
    const changedCode = prDetails.head_sha
      ? await buildChangedCodeContext(
          {
            headSha: prDetails.head_sha,
            files: prDetails.files.filter((f) =>
              CodeReviewPromptBuilder.isReviewableFile(f.filename),
            ),
          },
          githubChangedCodeDeps(
            repoDetails.owner,
            repoDetails.name,
            extra?.githubToken,
          ),
    ...
    if (changedCode) {
      console.log(
        `[Review] PR-head enclosing code @ ${changedCode.headSha.slice(0, 7)}:`,
        Object.fromEntries(
          changedCode.files.map((f) => [
            f.filename,
            `${f.coverage} (${f.chunks.length} chunk(s))`,
          ]),
        ),
      );
    }

    // Only files with coverage EXACTLY "guaranteed" skip the default-branch
    // changed_file fetch — plus their pre-rename path, since the index still
    // holds the old path's pre-change code for a renamed file.
    const fullyCovered = new Set(changedCode?.fullyHeadCoveredFiles ?? []);
    const fullyHeadCoveredFiles = prDetails.files
      .filter((f) => fullyCovered.has(f.filename))
      .flatMap((f) =>
    ...
    for (const f of prDetails.files) {
    ...
    const retrievedContext = await retrievalService.retrieveReviewContext(
      ...
      undefined,
      docQuery,
      { skipSync: extra?.skipSync, fullyHeadCoveredFiles },
    );
    ...
    const codebaseContext = codebaseChunks
      .map(
        (chunk) =>
          `// File: ${chunk.filePath}\n// Symbol: ${chunk.symbolName}\n// Retrieved as: ${chunk.symbolType}\n${stripChunkHeader(chunk.content)}`,
      )
      .join("\n\n");

    const classCounts = codebaseChunks.reduce<Record<string, number>>(
      (acc, c) => {
    ...
    const touchesDocumentation = changedFilePaths.some((p: string) =>
      isDocumentationFile(p),
    );
    ...
    docRetrievalLog(
      `Review "${prDetails.title}" (${repoDetails.name}): documentation trigger = ${docTriggerReason}. ` +
    ...
    docChunks.forEach((d, i) => {
      docRetrievalLog(
        `  -> included [Doc ${i + 1}] ${d.filePath} § "${d.sectionPath}"`,
      );
    });
    ...
    // Repository-level framing: the deterministic profile (purpose, modules,
    // runtime services, dependencies), already capped and keyed to the
    // indexed revision by retrieveReviewContext.
    const repositoryOverview = retrievedContext.repositoryProfile ?? "";
    ...
    return { messages, retrievedContext, changedCode };
  }
  ...
  async generateAndStoreReview(
    ...
    try {
      ...
      // 3. Retrieve context and build the prompt. getPullRequestDetails above
      // already required a working GitHub token; a failed second lookup only
      // disables private-repo head fetches (per-file fallback), never the review.
      const githubToken = await githubService
        .getGitHubAccessToken(clerkUserId)
        .catch(() => undefined);
      const { messages } = await this.buildReviewMessages(
        clerkUserId,
        repositoryId,
        repoDetails,
        prDetails,
...
export const getReviewsForPullRequest = async (
  ...
  const reviews = await withCache(
    `repo:${repositoryId}:pr:${pullNumber}:reviews`,
    300,
    async () => {
